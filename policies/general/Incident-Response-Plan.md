...
*Free to use, fork, and adapt at [github.com/sortedsol/Open-IT-Policy-Library](https://github.com/sortedsol/Open-IT-Policy-Library)* | [Download DOCX version](https://github.com/sortedsol/Open-IT-Policy-Library/raw/main/policies/general/Incident-Response-Plan.docx)

---

## 1. Purpose

This document is a starter incident response plan — a framework your company can customize, not a finished product. It outlines the basic steps every company should be prepared to take when a security incident occurs: a ransomware attack, a data breach, a compromised account, a lost device with client data, or any other event that threatens the company's data or systems.
...
This plan covers security incidents affecting the company's technology systems, data, and operations. This includes:

- Ransomware or malware infections
- Unauthorized access to company systems or accounts
- Data breaches involving client, business, or employee data
...
- Accidental data exposure (sending client data to the wrong recipient, uploading to an unapproved tool)
- Denial-of-service attacks affecting company systems
...
Every company needs an incident response team — even if it's just three people. These are the roles, not necessarily separate people. In a small company, one person may fill multiple roles.

### 3.1 Core Roles

| Role | Responsibility | Who |
|------|---------------|-----|
| **Incident Lead** | Makes decisions during the incident. Coordinates the response. Communicates with leadership. | [INSERT: typically IT director or senior manager] |
...
| **Communications Lead** | Handles client notification, employee communication, and external inquiries. | [INSERT: typically executive or operations manager] |
| **Legal Advisor** | Advises on notification requirements, regulatory obligations, and liability. | [INSERT: company's legal counsel — identify before an incident happens] |
...
Examples: active ransomware infection, confirmed data breach involving client data, compromised admin account with evidence of unauthorized access, active business email compromise with financial fraud in progress
...
Examples: compromised user account (not admin), malware detected and contained on one device, lost/stolen laptop with client data, phishing attack where credentials were entered but no unauthorized access detected yet
...
**Who reports:** Anyone at the company. Every employee should know how to report a suspected incident.
...
Remove the threat from company systems:
...
- Confirm that all client and business data is intact
...
1. **Check contracts.** Many client contracts have specific breach notification requirements and timelines.
...
Ransomware is the single biggest cyber threat to small and mid-sized businesses. If you're hit:
...
7. **Assess client deadlines.** If systems are down, which projects or client commitments are at risk? Communicate proactively about potential delays.
...
- **When the company's technology environment changes significantly** (new office, new cloud platform, new managed IT provider)
...
This incident response plan is a starting framework, not legal advice. It does not replace professional consultation with legal counsel, insurance advisors, or qualified cybersecurity professionals. Every company should customize this plan to its specific environment, obligations, and risk profile.
